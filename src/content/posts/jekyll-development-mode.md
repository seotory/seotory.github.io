...
`jekyll` [공식문서](https://jekyllrb.com/docs/configuration/#specifying-a-jekyll-environment-at-build-time)에 가보면 환경변수(env)를 통해 빌드시 선택적으로 빌드할 수 있도록 지원해주고 있다. `jekyll`에서는 공식적으로 `JEKYLL_ENV`이라는 명칭으로 사용한다. `JEKYLL_ENV` 환경변수에 운영이면 `production`를 입력하고 개발이면 `development`를 입력한다. 이렇게 작성된 환경변수를 `jekyll`에서 사용한다.
...
이 방법의 단점은 window에서는 작동하지 않는다는 점이다. window에서 사용하고 싶다면 npm의 [cross-env](https://www.npmjs.com/package/cross-env) 라이브러리를 글로벌로 설치해서 아래와 같이 사용하거나 비슷한 프로그램을 찾아서 이용하면 된다.
...
오늘 아웃사이더님의 블로그를 보니, 폴더별로 환경변수 관리를 해줄 수 있는 프로그램을 사용하고 있었다. [direnv](https://direnv.net/) 사이트를 참고해서 환경변수를 셋팅한다.
